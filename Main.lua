...
function Main.SyncBackgroundTasks()
    ...
    Core.ToggleTask("CollectFruit1", Options.tgCollectFruitEnable.Value, function() Farming.CollectFruitWorker(1) end)
    Core.ToggleTask("CollectFruit2", Options.tgCollectFruitEnable2.Value, function() Farming.CollectFruitWorker(2) end)
...
function Main.Init()
    ...
    Core.DataStream.OnClientEvent:Connect(function(Type, Profile, Data)
        ...
        for _, Packet in ipairs(Data) do
            ...
            -- Sync Shop Stocks
            if Shop.BuyList and Shop.BuyList[Key] then 
                task.spawn(function() Shop.ProcessBuy(Key, Content) end) 
            end
            ...
            if UI.Options.PetModeEnable.Value then
                task.spawn(function()
                    if string.find(Key, "ROOT/GardenGuide/PetData") or Key == "ROOT/BadgeData/PetMaster" then
                        if UI.Options.PetMode.Value == "Nightmare" then Pet.PetNightmare()
                        elseif UI.Options.PetMode.Value == "Mutant" then Pet.CheckMakeMutant()
                        else Pet.Mutation() end
