-- [[ EfHub Addon: Collapsible Section (Module Version) ]] --
...
return function(FluentLibrary)
	...
	local function GetNextLayoutOrder(Container)
		...
		for _, Child in ipairs(Children) do
			if Child:IsA("GuiObject") and Child.LayoutOrder > MaxOrder then MaxOrder = Child.LayoutOrder end
		end
		return MaxOrder + 1
	end
	...
	local function CreateCollapsibleSection(self, Title, Opened)
		...
		local function UpdateState()
			...
			for _, Item in ipairs(SectionElements) do
				if Item.Frame then Item.Frame.Visible = IsOpened end
			end
		end
		...
		-- [ 4. Wrapper Functions ]
		local function AddWrapper(FuncName)
			Section[FuncName] = function(self, ...)
				local args = { ... }
				return Section:Wrap(function()
					return ParentTab[FuncName](ParentTab, unpack(args))
				end)
			end
		end

		AddWrapper("AddToggle")
		AddWrapper("AddButton")
		AddWrapper("AddSlider")
		AddWrapper("AddDropdown")
		AddWrapper("AddInput")
		AddWrapper("AddParagraph")
		AddWrapper("AddKeybind")
		AddWrapper("AddColorpicker")
