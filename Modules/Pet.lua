...
function Pet.GetRawPetData(uuid)
    local success, result = pcall(function() return Core.ActivePetsService:GetPetData(Core.LocalPlayer.Name, uuid) end)
...
function Pet.GetPetMutation(uuid)
    ...
    if data and data.PetData then
        ...
        if rawEnum then return Pet.EnumToNameCache and Pet.EnumToNameCache[rawEnum] or "None"
...
function Pet.MakePetFavorite(uuid)
    ...
    if not Pet.GetPetFavorite(uuid) then
        ...
        for _, item in ipairs(Core.LocalPlayer.Backpack:GetChildren()) do
            if item:GetAttribute("ItemType") == "Pet" and item:GetAttribute("PET_UUID") == uuid then targetItem = item; break end
        end
        if targetItem then favoriteEvent:FireServer(targetItem); task.wait(0.3); return true end
    end
    return false
end
...
function Pet.GetPetUUID(petNameTable)
    ...
    elseif petMode == "Mutant" then targetMutant = UI.Options.TargetMutantDropdown.Value end
...
function Pet.EquipPet(uuid) Core.GameEvents:WaitForChild("PetsService"):FireServer("EquipPet", uuid) end
function Pet.UnequipPet(uuid) Core.GameEvents:WaitForChild("PetsService"):FireServer("UnequipPet", uuid) end
function Pet.SwapPetLoadout(Loadout)
    if Loadout == 2 then Loadout = 3 elseif Loadout == 3 then Loadout = 2 end
...
function Pet.MakeMutant(uuid)
    ...
    task.wait(0.2)
    if Pet.IsActivePet(uuid) then Pet.UnequipPet(uuid); task.wait(1) end
...
function Pet.Mutation(uuid)
    if UI.Options.PetModeEnable.Value then
        ...
        if Pet.targetUUID then
            ...
            if IsEquipPet() then
                local MyFarm = require(script.Parent.Farming).GetMyFarm()
                local FarmPoint = MyFarm and MyFarm:FindFirstChild("Spawn_Point") and MyFarm.Spawn_Point.CFrame
                if FarmPoint then Core.Character:PivotTo(CFrame.new(FarmPoint.X, FarmPoint.Y, FarmPoint.Z)); task.wait(0.3) end
                Pet.SwapPetLoadout(UI.Options.LevelSlots.Value)
                task.wait(UI.Options.LoadOutDelay.Value)
                pcall(function() Pet.EquipPet(Pet.targetUUID) end)
...
function Pet.PetNightmare()
    ...
    if mutant and mutant ~= "Nightmare" then
        ...
        for _, container in ipairs(petsPhysical:GetChildren()) do
            ...
            if PetModel then
                require(script.Parent.Farming).heldItemName("Cleansing Pet Shard")
        ...
        if hunger <= UI.Options.PetHungerPercent.Value then
            ...
            for invUuid, Item in pairs(inv) do
                if Item.ItemType == "Holdable" and Item.ItemData and not Item.ItemData.IsFavorite then
                    ...
                    if UI.Options.AllowAllFood.Value or table.find(allowList, FruitInv) then fruitUUID = invUuid; break end
                end
            end
            if fruitUUID and require(script.Parent.Farming).heldItemName(inv[fruitUUID].ItemData.ItemName) then
...
function Pet.processAgeBreakMachine()
    ...
    pcall(function()
        if machineData.PetReady then Core.GameEvents.PetAgeLimitBreak_Claim:FireServer(); task.wait(1.5); return end
...
function Pet.ValidEggs(EggsData, rEggs)
    ...
    for _, rEgg in pairs(rEggs) do
        ...
        if EggData then
            ...
            if spWeight ~= 0 and HatchWeight >= spWeight then table.insert(spEggs, rEgg)
            elseif #spTypeList > 0 and table.find(spTypeList, EggData.Data.Type) then table.insert(spEggs, rEgg)
            else table.insert(nmEggs, rEgg) end
        end
    end
...
function Pet.HatchEgg()
    ...
    if not fData or type(fData) ~= "table" then Pet.isEggProcessing = false; return end
    local PetsData, petCount, ReadyEggs = {}, 0, {}
    for Key, PetData in pairs(fData) do
        if PetData.Data.CanHatch then PetsData[Key] = PetData; petCount = petCount + 1 end
    end
    for _, nEggs in pairs(myEggs) do
        if nEggs:GetAttribute("READY") and (table.find(HatchList, "ALL") or table.find(HatchList, nEggs:GetAttribute("EggName"))) then
            table.insert(ReadyEggs, nEggs)
        end
    end
    if petCount ~= #ReadyEggs then Pet.isEggProcessing = false; return end

    local NormalEggs, SpecialEggs = Pet.ValidEggs(PetsData, ReadyEggs)
    if #NormalEggs > 0 then
        Pet.SwapPetLoadout(tonumber(UI.Options.ddHatchSlot.Value)); task.wait(10)
        for _, rEgg in pairs(NormalEggs) do Core.GameEvents.PetEggService:FireServer("HatchPet", rEgg); task.wait(tonumber(UI.Options.ipHatchDelay.Value)) end
    end
    task.wait(10)
    if #SpecialEggs > 0 then
        Pet.SwapPetLoadout(tonumber(UI.Options.ddSpecialHatchSlot.Value)); task.wait(10)
        for _, sEgg in pairs(SpecialEggs) do Core.GameEvents.PetEggService:FireServer("HatchPet", sEgg); task.wait(tonumber(UI.Options.ipHatchDelay.Value)) end
    end
    pcall(function() Core.Humanoid:UnequipTools() end)
    Pet.isEggProcessing = false
end
...
function Pet.PlaceEggs()
    ...
    if #farmEgg >= tonumber(UI.Options.ipMaxEggs.Value) then
        ...
        if lo ~= tonumber(UI.Options.ddSpeedEggSlot.Value) then Pet.SwapPetLoadout(tonumber(UI.Options.ddSpeedEggSlot.Value)); task.wait(5) end
        EggMultiple = 0; return
    end
    Pet.isEggProcessing = true
    local Plate = Pet.getPlate()
    local Boundary = Pet.getBoundary(Plate[2])
    local x, y, z = Boundary.maxX, 0.1355266571044922, Boundary.minZ
    z = z + (4 * EggMultiple); EggMultiple = EggMultiple + 1
    if z > Boundary.maxZ then x = x - 4; z = Boundary.minZ; EggMultiple = 1 end
    if x < Boundary.minX then Pet.isEggProcessing = false; return end
    local NewPos = (Boundary.cf * CFrame.new(x, 0, z)).Position
    
    local Farming = require(script.Parent.Farming)
    Farming.heldItemName(PlaceList[Random.new():NextInteger(1, #PlaceList)])
    task.wait(0.1)
...
function Pet.IsValidSellPet(petData)
    ...
    if not UI.Options.tgSellMutantPet.Value and sMutant ~= "m" then return false
    elseif sSellWeight ~= 0 and UI.Options.ddSellWeightMode.Value == "Below" and base >= sSellWeight then return false
    elseif sSellWeight ~= 0 and UI.Options.ddSellWeightMode.Value == "Above" and base <= sSellWeight then return false
    elseif sSellMode == "Black list" and table.find(typeList, petData.PetType) then return false
    elseif sSellMode == "White list" and not table.find(typeList, petData.PetType) then return false end
    return true
end
...
function Pet.Init(RefCore, RefUI)
    ...
    for mutantName, mutantInfo in pairs(MutantData["PetMutationRegistry"]) do
        if type(mutantInfo) == "table" and mutantInfo.EnumId then
            Pet.EnumToNameCache[mutantInfo.EnumId] = mutantName
        end
    end
...
function Pet.BuildUI()
    ...
    PetWorkSection:AddDropdown("PetMode", { Title = "Pet Mode", Values = { "Nightmare", "Elephant", "Mutant", "Level" }, Default = "Nightmare", Callback = function(Value) Core.QuickSave(); Sync() end })
    PetWorkSection:AddToggle("PetModeEnable", { Title = "Enable Pet Farm", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetWorkSection:AddDropdown("TargetPetDropdown", { Title = "Target Pet", Values = PetTable, Multi = true, Default = {}, Searchable = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetWorkSection:AddToggle("UseFavoriteOnly", { Title = "Use Favorite Pet Only", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetWorkSection:AddDropdown("TargetMutantDropdown", { Title = "Target Mutant", Values = MutantTable, Multi = false, Default = "", Searchable = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetWorkSection:AddInput("AgeLimitInput", { Title = "Age Limit", Default = 50, Filter = "Number", Callback = function(Value) Core.QuickSave(); Sync() end })
    PetWorkSection:AddInput("LoadOutDelay", { Title = "Loadout Switch Delay time", Default = 10, Filter = "Number", Callback = function(Value) Core.QuickSave(); Sync() end })
    PetWorkSection:AddDropdown("LevelSlots", { Title = "Select Loadout", Values = { 1, 2, 3, 4, 5, 6 }, Default = 1, Multi = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetWorkSection:AddDropdown("TimeSlots", { Title = "Select Time Slot", Values = { 1, 2, 3, 4, 5, 6 }, Default = 2, Multi = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetWorkSection:AddDropdown("MutantSlots", { Title = "Select Mutant Slot", Values = { 1, 2, 3, 4, 5, 6 }, Default = 3, Multi = false, Callback = function(Value) Core.QuickSave(); Sync() end })

    local AutoAgeBreakSection = Tabs.Pet:AddCollapsibleSection("Auto Age Break")
    AutoAgeBreakSection:AddToggle("AAB_Enabled", { Title = "Enable Auto Age Break", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    AutoAgeBreakSection:AddDropdown("AAB_PetType", { Title = "Select Pet Type", Values = PetTable, Default = "", Multi = false, Searchable = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    AutoAgeBreakSection:AddInput("AAB_TargetAge", { Title = "Target Break Age", Default = 125, Numeric = true, Finished = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    AutoAgeBreakSection:AddToggle("AAB_CheckWeight", { Title = "Check Dupe Weight?", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    AutoAgeBreakSection:AddDropdown("AAB_WeightCond", { Title = "Weight Condition", Values = { "<=", ">=" }, Default = "<=", Callback = function(Value) Core.QuickSave(); Sync() end })
    AutoAgeBreakSection:AddInput("AAB_WeightVal", { Title = "Dupe Weight Value", Default = 10, Numeric = true, Finished = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    AutoAgeBreakSection:AddToggle("AAB_CheckAge", { Title = "Check Dupe Age?", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    AutoAgeBreakSection:AddDropdown("AAB_AgeCond", { Title = "Age Condition", Values = { "<=", ">=" }, Default = "<=", Callback = function(Value) Core.QuickSave(); Sync() end })
    AutoAgeBreakSection:AddInput("AAB_AgeVal", { Title = "Dupe Age Value", Default = 30, Numeric = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    ...
    PetFeedSection:AddToggle("AutoFeedPet", { Title = "Auto Feed", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetFeedSection:AddToggle("AllowAllFood", { Title = "Allow All Food", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetFeedSection:AddDropdown("AllowFoodType", { Title = "Allow Food Type", Values = FruitTable, Multi = true, Default = {}, Searchable = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetFeedSection:AddSlider("PetHungerPercent", { Title = "Pet Hunger Percent", Min = 1, Max = 100, Default = 80, Rounding = 1, Callback = function(Value) Core.QuickSave(); Sync() end })

    local PetGiftSection = Tabs.Pet:AddCollapsibleSection("Auto Accept Pet gift", false)
    PetGiftSection:AddToggle("tgAcceptPetGift", { Title = "Enable Auto Accept", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    PetGiftSection:AddInput("inPetGiftDelay", { Title = "Accept Delay (s)", Default = 0.1, Filter = "Number", Callback = function(Value) Core.QuickSave(); Sync() end })

    local HatchSection = Tabs.Pet:AddCollapsibleSection("Auto Hatch Eggs", false)
    HatchSection:AddToggle("tgPlaceEggsEn", { Title = "Place Eggs", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    local AllEggTable = {}
    ...
    HatchSection:AddDropdown("ddPlaceEgg", { Title = "Select Eggs", Values = AllEggTable, Multi = true, Default = {}, Searchable = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddInput("ipMaxEggs", { Title = "Max Eggs", Default = 3, Numeric = true, Finished = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddDropdown("ddSpeedEggSlot", { Title = "Select Speed Loadout", Values = { 1, 2, 3, 4, 5, 6 }, Default = 1, Multi = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddInput("ipPlaceEggDelay", { Title = "Place Eggs Delay", Default = 0.2, Numeric = true, Finished = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddToggle("tgAutoHatchEn", { Title = "Auto Hatch", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    local tempTable = { "ALL" }
    for i, v in ipairs(AllEggTable) do table.insert(tempTable, v) end
    HatchSection:AddDropdown("ddEggHatch", { Title = "Select Egg to Hatch", Values = tempTable, Multi = true, Default = { "ALL" }, Searchable = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddDropdown("ddHatchSlot", { Title = "Select Hatch Loadout", Values = { 1, 2, 3, 4, 5, 6 }, Default = 2, Multi = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddInput("ipHatchDelay", { Title = "Hatch Egg Delay", Default = 0.2, Numeric = true, Finished = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddInput("ipSpecialHatchWeight", { Title = "Special Hatch Weight", Default = 0, Numeric = true, Finished = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddDropdown("ddSpecialHatchType", { Title = "Special Hatch Pet", Values = PetTable, Multi = true, Default = {}, Searchable = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddDropdown("ddSpecialHatchSlot", { Title = "Select Hatch Loadout", Values = { 1, 2, 3, 4, 5, 6 }, Default = 4, Multi = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddToggle("tgSellPetEn", { Title = "Auto Sell Pet", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddDropdown("ddSellPetSlot", { Title = "Select Sell Pet Loadout", Values = { 1, 2, 3, 4, 5, 6 }, Default = 3, Multi = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddDropdown("ddSellPetType", { Title = "Sell Pet Type", Values = PetTable, Multi = true, Default = {}, Searchable = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddDropdown("ddSellMode", { Title = "Sell Pet Mode", Values = { "ALL", "White list", "Black list" }, Default = "White list", Multi = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddInput("ipSellWeight", { Title = "Sell Pet Weight", Default = 0, Numeric = true, Finished = true, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddDropdown("ddSellWeightMode", { Title = "Sell Weight Mode", Values = { "Below", "Above" }, Default = "Below", Multi = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddToggle("tgSellMutantPet", { Title = "Sell Mutant Pet", Default = false, Callback = function(Value) Core.QuickSave(); Sync() end })
    HatchSection:AddInput("ipSellPetDelay", { Title = "Sell Pet Delay", Default = 0.2, Numeric = true, Finished = true, Callback = function(Value) Core.QuickSave(); Sync() end })
end
